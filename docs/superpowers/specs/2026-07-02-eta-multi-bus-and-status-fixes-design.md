...
- `StopStatus == 0`(正常),且
- `EstimateTime` 不為 `None`,且
- `PlateNumb` 非空字串(TDX 文件:尚未發車者 PlateNumb 為空)。
...
- 尚未發車(無車牌)的筆數:不列入(使用者可搭其他台)。

**B. 可搭乘 == 1 台(單車,含車牌):維持單行**
- 行駛中:`🚌 70左 - EAA-732 預估 8 分鐘到「臺南高工」`
- 即將進站(≤60 秒):`🚌 70左 - EAA-732 即將進站到「臺南高工」`
...
- **Bug A**:狀態 0 + `EstimateTime == 0` → 「即將進站」,非系統異常。
