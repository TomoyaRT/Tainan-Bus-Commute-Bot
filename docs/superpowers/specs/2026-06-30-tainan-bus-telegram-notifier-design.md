...
- **硬性原則：完全免費，絕不為本服務付費**（不購買任何點數）。
- 「3 點」免費額度結論：
  - 舊模式（多數會員仍適用）：每把金鑰 50 次/秒、無每日上限 → 綽綽有餘。
  - 點數新模式：3 點/月。若公車即時動態屬「基礎服務」(1 點=1500 次) → 4500 次/月，足夠；若屬「進階服務」(1 點=200 次) → 600 次/月，略不足。**此情況下不付費**，改以「縮短窗口／拉長間隔」把用量壓進免費額度；並靠下方的「失敗即停」與「手動/自動停止」進一步省額度。
- 頻率限制（最壞每分鐘 5 次）對「每 5～10 分鐘一次」的分散呼叫無影響。
- **行動項**：部署前到 TDX 會員專區確認自己帳號實際適用的額度模式；若為進階點數模式，依上述以調整間隔/窗口控制，而非付費。
...
```
{
  morning: { stopped: bool, intervalOverride: number|null, lastPushAt: timestamp|null, failCount: number },
  evening: { stopped: bool, intervalOverride: number|null, lastPushAt: timestamp|null, failCount: number }
}
```
- `stopped`：手動或自動停 → true；隔天為新文件，自動恢復。
- `intervalOverride`：當天「推播間隔」按鈕設的值；隔天失效，回到 `defaultInterval`。
- `lastPushAt`：用於計算下一次是否到期。
- `failCount`：當天該時段「連續」TDX 呼叫失敗次數；成功時歸零，達門檻即停（見第 6 節）。
...
5. 呼叫 TDX：
   - **成功**：`failCount` 歸零 → 取目標站 ETA → 套狀態文案 → `sendMessage`（附 2 顆 inline 按鈕）→ 更新 `lastPushAt`。
   - **失敗**（HTTP 錯誤、逾時、回應無法解析等）：`failCount += 1`，**不推播**（避免每次失敗都浪費額度與洗版）。
     - 若 `failCount < 2`：本次靜默，等下個 tick 再試。
     - 若 `failCount` 達 **2**（連續第 2 次失敗）：**主動推一次** `⚠️ 政府API出狀況，暫時無法取得正確的資訊。`，並將該時段標記 `stopped`（當天不再呼叫，隔天自動恢復）。回饋訊息全程只推一次。
6. 現在 ≥ 窗口結束時間（09:30 / 21:00）→ 標記該時段 `stopped`（自動停）。

第一推對齊窗口起點：5 分鐘 tick 對齊時鐘，08:00 / 18:30 的 tick 觸發第一推。

> 失敗門檻說明：採「連續第 2 次失敗即停」，總共最多浪費 2 次呼叫，最省額度。`failCount` 為「連續」計數，任何一次成功即歸零。
...
| API 異常/拉不到 | 連續第 2 次呼叫失敗（見第 6 節） | `⚠️ 政府API出狀況，暫時無法取得正確的資訊。`（只推一次後停該時段） |

- ETA 分鐘以 `ceil(EstimateTime/60)` 計。
- 「站名」「路線」依該時段設定動態帶入。
- 「拉不到資料」僅在連續第 2 次失敗時推一次回饋並停止該時段，不每次失敗都推、不浪費額度。
